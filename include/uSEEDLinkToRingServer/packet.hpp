#ifndef USEED_LINK_TO_RING_SERVER_PACKET_HPP
#define USEED_LINK_TO_RING_SERVER_PACKET_HPP
#include <vector>
#include <chrono>
#include <memory>
#include <string>
...
public:
    ...
    /// @param[in] startTime  The UTC start time in nanooseconds
    ///                       from the epoch (Jan 1, 1970). 
    void setStartTime(const std::chrono::nanoseconds &startTime) noexcept;
    /// @result The UTC start time in nanoseconds from the epoch.
    [[nodiscard]] std::chrono::nanoseconds getStartTime() const noexcept;
    /// @result The UTC time in nanoseconds from the epoch of the last sample.
    ...
    /// @result A pointer to the underlying data.
    /// @note This exists for optimization purposes - it is recommended to
    ///       use getData().
...
[[nodiscard]] double computeSumOfSamples(const Packet &packet);
[[nodiscard]] double computeSumOfSamplesSquared(const Packet &packet);
...
/// @result Converts the packet to MiniSEED records to string buffers.
[[nodiscard]] std::vector<DataLinkPacket>
